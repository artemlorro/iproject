...
class FieldImages extends FieldAbstract
{
...
//		}
	...
	{
		$uploaddir = Yii::app()->basePath . '/../' . $paramField['dirname'] . '/';
		...
		$imageParam = getimagesize($_FILES[$uploadFieldName]['tmp_name']);

		if (strpos($imageParam['mime'], 'image') === false) {
			throw new A_Exception("A_Admin_Model_FieldType_Images::UPLOAD_NOT_IMAGE", 'Ошибка! Этот файл не является изображением!');
		}
		...
		if (move_uploaded_file($_FILES[$uploadFieldName]['tmp_name'], $file)) {
			$w = isset($paramField['w']) && (int) $paramField['w'] ? $paramField['w'] : 900;
			$h = isset($paramField['h']) && (int) $paramField['h'] ? $paramField['h'] : 900;
			$this->imageResize($file, false, $w, $h);
			$preview = $this->getImagePreview($paramField, $filename);
//			if (isset($paramField['tmb'])) $this->getImagePreview($paramField, $filename, $paramField['tmb']);
			return array('filename' => $filename, 'preview' => $preview);
		} else {
			throw new A_Exception("A_Admin_Model_FieldType_Images::NOT_UPLOAD", 'Ошибка! Файл не загружен!');
		}
	}

	function addFile ($value, $fileName = false)
	{
		if (!$fileName) return $value;
		return $value . (trim($value) ? ';' : '') . $fileName;
	}
	...
	function deleteFile ($value, $fileName, $paramField)
	{
		...
		if (in_array($fileName, $files)) {
			...
			$path = Yii::app()->basePath . '/../' . $paramField['dirname'] . '/';
			$pathFile = $path . $fileName;
			$pathPreview = $path . 'p_' . $fileName;
			$pathTmb = $path . 'tmb_' . $fileName;
			if (file_exists($pathFile)) unlink($pathFile);
			if (file_exists($pathPreview)) unlink($pathPreview);
			if (file_exists($pathTmb)) unlink($pathTmb);
		}
